          ...
          <div className="hide-scrollbar max-h-[85vh] w-full max-w-3xl overflow-y-auto rounded-[30px] bg-[#e3fff1] p-8 shadow-2xl">
                ...
                <div className="flex items-start justify-between gap-6">
                  <div>
                    ...
                    <p className="mt-2 text-text-secondary">
                      {filteredSlots[0]?.powerKw ?? power} kW · Rp{" "}
                      {(filteredSlots[0]?.pricePerKwh ?? pricePerKwh).toLocaleString(
                        "id-ID"
                      )}{" "}
                      / kWh
                    </p>
                    ...
                    filteredSlots.map((slot) => {
                      const getSlotStatusColor = () => {
                        switch (slot.status) {
                          ...
                          case "MAINTENANCE":
                            return "bg-maintenance2 text-white";
            ...
            {modalStep === "schedule" && selectedSlot && (
              <ChargingSlotCard
                ...
                type={type}
                power={selectedSlot.powerKw}
                pricePerKwh={selectedSlot.pricePerKwh}
                onBack={() => {
                  setSelectedSlot(null);
                  setModalStep("slots");
                }}
